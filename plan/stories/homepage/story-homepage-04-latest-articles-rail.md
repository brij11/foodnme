...
## Acceptance criteria
- [ ] "Latest from the blog" rail renders the **4** most-recent published articles as compact title links, ordered by `published_at desc`, **excluding whichever article is shown as the editorial feature** (`story-homepage-06`, §3.6 #4) so no article appears twice
- [ ] Rail mounts beneath the editorial feature inside the §3.6 #4 "From the Knowledge Hub" section — co-located, **not** a standalone section; it does not duplicate `story-homepage-06`'s single editorial feature, and does **not** re-render the section header / "All articles → /blog" link (those belong to the shell/feature)
- [ ] Each rail item links to `/blog/[slug]`; the section's existing "All articles → /blog" link is retained (rail complements it)
- [ ] Respects `prefers-reduced-motion` (stagger interaction policy, `UI-DESIGN-HANDOFF.md` §4.10)
- [ ] Articles fetched server-side at build/revalidate (SSG snapshot) — no client DB call blocks initial render; homepage still meets LCP < 2.0s after wiring
- [ ] Short-list tolerance: if fewer than 4 published articles exist, the rail renders only those available (no placeholder rows) per §5.4

## Tasks
- [new] Query the 4 most-recent published articles (`published_at desc`) server-side within the SSG page data (build/revalidate), tolerating fewer than 4 results
- [new] Render the "Latest from the blog" rail as compact `/blog/[slug]` title links beneath the editorial feature inside the §3.6 #4 section — no duplicate section header or "All articles →" link
- [new] Apply stagger entrance honoring `prefers-reduced-motion` (§4.10)
- [new] Verify LCP < 2.0s post-wiring and the short-list case (renders only available items, no placeholders)
