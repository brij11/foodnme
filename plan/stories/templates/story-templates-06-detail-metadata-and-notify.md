...
## Acceptance criteria
- [ ] The detail page shows a metadata strip with Format / Last updated / Downloads (Pages omitted — no schema column) (DEVIATIONS B14; `web/components/templates/WhatsIncluded.tsx:20` vs `design/screens-main.jsx:638`)
- [ ] The "notify on revision" affordance is a clear control with submit + success/error microcopy ("You'll be the first to know.") (DEVIATIONS B15; `web/components/templates/TemplateDownloadPanel.tsx:71` vs `design/screens-main.jsx:662`)
- [ ] Notify submission reuses the existing newsletter/email-capture path (no new endpoint required)
- [ ] No regression to the faithful detail pieces (breadcrumb, What's-Inside list, request-customization CTA, sticky download panel, similar-templates rail)
...
- [started] Add/adjust tests: metadata strip renders + notify success/error states (AC 1, 2)
