...
export const BLOG_POPULAR_TAGS: readonly string[] = [
  "HACCP", "FSSAI", "Allergens", "Sampling", "Validation", "Labeling", "Microbiology", "Thermal",
];
...
export function articleCategory(slug: string): Category | undefined { return lookup(ARTICLE_CATEGORIES, slug); }
export function templateCategory(slug: string): Category | undefined { return lookup(TEMPLATE_CATEGORIES, slug); }
export function articleCategoryLabel(slug: string): string { return articleCategory(slug)?.label ?? slug; }
export function templateCategoryLabel(slug: string): string { return templateCategory(slug)?.label ?? slug; }
export function articleTagVariant(slug: string): TagVariant { return articleCategory(slug)?.tag ?? "green"; }
export function templateTagVariant(slug: string): TagVariant { return templateCategory(slug)?.tag ?? "green"; }
